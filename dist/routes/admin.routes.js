...
router.patch("/tickets/:id/verify", verifyJWT, verifyAdmin, validate({ params: objectIdParamSchema, body: verifyTicketSchema }), async (req, res) => {
...
router.patch("/tickets/:id/advertise", verifyJWT, verifyAdmin, validate({ params: objectIdParamSchema }), async (req, res) => {
    const ticketId = parseObjectId(req.params.id, "ticket id");
    const db = await connectDB();
