...
import type { JwtPayload, UserDoc, UserRole } from "../types/models.js";

const router = Router();

function createToken(user: { email: string; role?: UserRole; name: string }): string {
...
router.post("/login", validate({ body: loginSchema }), async (req, res) => {
  ...
  if (user.provider === "google") {
    throw new AppError(
      401,
      "This email uses Google login. Please click 'Continue with Google'"
    );
  }
