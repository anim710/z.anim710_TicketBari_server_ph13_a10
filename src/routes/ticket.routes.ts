...
import type { TicketDoc, UserDoc } from "../types/models.js";
...
function stripImmutableFields(body: Record<string, unknown>): Record<string, unknown> {
  const cleaned = { ...body };
...
router.get("/", validate({ query: ticketListQuerySchema }), async (req, res) => {
  ...
  const { from, to, type, sort, page } = req.query as unknown as {
    from?: string;
    to?: string;
    type?: string;
    sort?: "low" | "high";
...
router.post(
  ...
  async (req, res) => {
    ...
    if (vendor?.isFraud) throw new AppError(403, "Account suspended");

    const body = stripImmutableFields(req.body as Record<string, unknown>);
...
router.patch(
  "/:id",
  verifyJWT,
  validate({ params: objectIdParamSchema, body: updateTicketSchema }),
  async (req, res) => {
    const db = await connectDB();
